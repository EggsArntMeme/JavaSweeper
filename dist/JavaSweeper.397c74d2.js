...
class Square {
    ...
    collectSuroundings() {
        ...
        if (!this.cords[0] - 1 < 0) {
            ...
            if (this.cords[1] - 1 >= 0) this.surroundingBoxes.push(this.parentList[this.cords[0] - 1][this.cords[1] - 1]);
            ...
            if (this.cords[1] + 1 < dim[1]) this.surroundingBoxes.push(this.parentList[this.cords[0] - 1][this.cords[1] + 1]);
        }
        if (this.cords[1] - 1 >= 0) this.surroundingBoxes.push(this.parentList[this.cords[0]][this.cords[1] - 1]);
        if (this.cords[1] + 1 < dim[1]) this.surroundingBoxes.push(this.parentList[this.cords[0]][this.cords[1] + 1]);
    ...
    checkForBombs() {
        if (!this.visible) this.surroundingBoxes.forEach((box)=>{
            if (box.isBomb) this.numberOfBombs += 1;
        });
    }
    revealSelf() {
        if (!this.flagged) {
...
for(let i = 0; i < dim[0]; i++){
    let rowList = [];
    for(let j = 0; j < dim[1]; j++)rowList.push(new Square(false, [
        i,
        j
    ], masterList));
...
function setBombs(firstClickedBox) {
    for(let i = 0; i < numOfTotalBombs; i++)while(true){
        let randomCord = [
            getRandomInt(dim[0]),
            getRandomInt(dim[1])
        ];
...
for(let i = 0; i < masterList.length; i++)for(let j = 0; j < masterList[i].length; j++)masterList[i][j].collectSuroundings();
                    ...
                    if (!flagToggle && boxDiv.innerHTML == '') {
                        if (!clickedBox.isBomb) {
                            ...
                            masterList.forEach((row)=>{
                                row.forEach((box)=>{
                                    if (box.isBomb) box.visible = true;
                                });
                            });
...
function renderBoxes() {
    ...
    for(let i = 0; i < body.children.length; i++){
        for(let j = 0; j < body.children[i].children.length; j++)if (masterList[i][j].visible) {
            ...
            } else if (!masterList[i][j].numberOfBombs && !numOfTotalBombs.isBomb) score += 10;
            if (masterList[i][j].isBomb) body.children[i].children[j].classList.add('apple');
            else body.children[i].children[j].classList.add('safe');
        }
...
window.addEventListener('keydown', (e)=>{
    switch(e.key){
        case 'r':
            for(let i = 0; i < masterList.length; i++)for(let j = 0; j < masterList[i].length; j++){
    ...
    if (e.key == 'r') {
        ...
        if (flagToggle) flagToggleBtn.classList.add('toggle-true');
        else flagToggleBtn.classList.remove('toggle-true');
    }
});
window.addEventListener('mousedown', (e)=>{
    ...
    } else if (e.button == 1) location.reload();
});
...
function winCheck() {
    ...
    masterList.forEach((row)=>{
        row.forEach((box)=>{
            if (!box.visible && !box.isBomb) winCondition = false;
        });
    });
    if (winCondition && numberOfFlags == 0) return true;
    else return false;
}
// TIMER / CLOCK
let time = [
...
},{"./api":"jDkOf"}],"jDkOf":[function(require,module,exports,__globalThis) {
...
},{}]},["MAtcu","agb61"], "agb61", "parcelRequireed5e", {})

//# sourceMappingURL=JavaSweeper.397c74d2.js.map
